...
extern RC readNextBlock (SM_FileHandle *fHandle, SM_PageHandle memPage){
	...
	if(fHandle->totalNumPages == current_page){ //condition to check if current position of file is on last page.
		...
		int start= (PAGE_SIZE*current_page); // storing the next page start position
...
extern RC writeCurrentBlock (SM_FileHandle *fHandle, SM_PageHandle memPage){
	...
	if(file_pointer == NULL)
		return RC_FILE_NOT_FOUND; //returns an Error message if the file pointer points to Null Value
	long int cur_Pos = fHandle->curPagePos;//Storing current position of file
	
	appendEmptyBlock(fHandle); //Appending an empty block to accomodate new content

	fseek(file_pointer,cur_Pos,SEEK_SET); //Seek to the current position.
...
extern RC appendEmptyBlock (SM_FileHandle *fHandle){
	SM_PageHandle Empty_Page = (SM_PageHandle)calloc(PAGE_SIZE,sizeof(char)); //creating empty page of PAGE_SIZE bytes
	fseek(file_pointer, 0, SEEK_END);
	fwrite(Empty_Page,sizeof(char),PAGE_SIZE,file_pointer); //Writing Empty page to the file.
	free(Empty_Page); //free memory 
