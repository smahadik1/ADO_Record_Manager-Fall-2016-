...
extern RC initRecordManager (void *mgmtData){
       initStorageManager(); // initializing Storage Manager inside Record Manager
       return RC_OK;
}
...
extern RC createTable (char *name, Schema *schema){
    ...
    t = (Table_Info*) malloc( sizeof(Table_Info) ); // Allocating memory for table data
    ...
    char data[PAGE_SIZE];
       ...
       *(int*) page_hdl = 0; // Total Number of tuples
    page_hdl+= sizeof(int); //incrementing the pointer
       
       *(int*)page_hdl = 1; // As Page 0 is for schema and other info so the first free page is 1.
    page_hdl += sizeof(int); 
       
       *(int*) page_hdl = schema->numAttr; //set number of attributes
   page_hdl += sizeof(int); 
 
    *(int*)page_hdl = schema->keySize; // set keySize for Attributes
    page_hdl += sizeof(int);
       
       for(i=0; i<schema->numAttr; i++)
    {
       
       strncpy(page_hdl, schema->attrNames[i], 10);       // setting the Attribute names
       page_hdl += 10;
 
       *(int*) page_hdl = (int)schema->dataTypes[i];      // Setting the Data Types of Attribute
       page_hdl += sizeof(int);
 
       *(int*) page_hdl = (int) schema->typeLength[i];    // Setting the typeLength of Attribute
       page_hdl += sizeof(int);
 
    }
              
       rc = createPageFile(name);          // Creating file for table using Storage Manager Function
       if(rc != RC_OK)
        return rc;
              
       rc = openPageFile(name, &fh); // opening the Created File
       if(rc != RC_OK)
        return rc;
              
       rc=writeBlock(0, &fh, data); // Writing Schema To 0th page of file
       if(rc != RC_OK)
        return rc;
              
       rc=closePageFile(&fh);                     // Closing File
       if(rc != RC_OK)
        return rc;
...
extern RC openTable (RM_TableData *rel, char *name){
    ...
    rel->name = strdup(name);               // Set mgmtData 
       ...
       t->no_of_rows= *(int*) page_hdl; // retrieving the total number of tuples from file
      
    page_hdl += sizeof(int);
    
       t->first_free_page= *(int*) page_hdl;     // retrieving the free page
       
    page_hdl += sizeof(int);
       
    numAttrs = *(int*) page_hdl;            // retrieving the number of Attributes 
       page_hdl += sizeof(int);
    ...
    for(i=0; i < numAttrs; i++)
       schema->attrNames[i]= (char*) malloc(10); //maximum field length is 10
...
extern RC closeTable (RM_TableData *rel){
       Table_Info *t;;
       t = rel->mgmtData;    // setting rel->mgmtData value to t before Closing
       shutdownBufferPool(&t-> buffer_mgr); //  Shutdown Buffer Pool
       rel->mgmtData = NULL; 
       return RC_OK;
}
 
// Deleting the Table file
