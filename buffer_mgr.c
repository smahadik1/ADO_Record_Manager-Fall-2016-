...
int buffer_sz = 0, rear = 0, write_count = 0, hit = 0, clock_pointer = 0,lfu_pointer = 0;
	...
	for(i=0;i<buffer_sz;i++)
	{
		if(ds[front].fixed_count == 0)
		{
			if(ds[front].dirty_bit == 1)
			{
				...
				writeBlock (ds[front].pno, &file_handle, ds[front].data);
				write_count++;
			}
	...
	for(;;)
	{
		...
		if(ds[clock_pointer].hitrate == 0)
		{
			if(ds[clock_pointer].dirty_bit == 1)
			{
				SM_FileHandle file_handle;
				openPageFile (bm->pageFile, &file_handle);
				writeBlock (ds[clock_pointer].pno, &file_handle, ds[clock_pointer].data);
				write_count++;
			}			
...
RC forceFlushPool(BM_BufferPool *const bm)
{
	...
	while(i<buffer_sz)
	{
		if(ds[i].fixed_count == 0 && ds[i].dirty_bit == 1)
		{
			SM_FileHandle file_handle;
			openPageFile (bm->pageFile, &file_handle);
			writeBlock (ds[i].pno, &file_handle, ds[i].data);
			ds[i].dirty_bit = 0;
			write_count++;
		}
		i++;
	    ...
	    const PageNumber pageNum)
{
	...
	if(ds[0].pno == -1)
	{
		...
		readBlock(pageNum, &file_handle, ds[0].data);
		ds[0].pno = pageNum;
		ds[0].fixed_count++;
		rear = 0;
		hit = 0;
	...
	{	
		...
		while(i<buffer_sz)
		{
			...
			else	//To check if the buffer has space to add a page
			{
				...
				readBlock(pageNum, &file_handle, ds[i].data);
				ds[i].pno = pageNum;
				ds[i].fixed_count = 1;
				ds[i].refnum = 0;
				rear++;	
		...
		if(check == 0)//To Check if the buffer is full and which replacement strategy to apply
		{
			...
			readBlock(pageNum, &file_handle, temp->data);
			temp->pno = pageNum;
			temp->dirty_bit = 0;		
			temp->fixed_count = 1;
			temp->refnum = 0;
